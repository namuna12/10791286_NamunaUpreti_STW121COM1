...
class WinAndFrame:
    def __init__(self, win):
        ...
        self.win.title('!!!!!!!!!!!! simple Calculator !!!!!!!!!!!!!!')  # Title of the window.
...
class MenuAndButton(WinAndFrame):
    ...
    def creation_credits():
        ...
        mb.showinfo('Credits', 'Credits goes to \shabda kafle \nAnkit Shah \parash aryal \nSamyog Shah')
    ...
    def equal(self):
        ...
        def namuna():
            ...
            namu = n.readlines()
            upreti = str(namu[0])
            if upreti < str(self.text):
                n.close()
                namu = open('total.txt', 'w')
                namu.write(str(self.text))
                namu.close()
                return self.text
            return upreti
        namuna()
